...
public class CommandWorldBorder {
    ...
    private static final SimpleCommandExceptionType ERROR_SAME_CENTER = new SimpleCommandExceptionType(IChatBaseComponent.translatable("commands.worldborder.center.failed"));
    private static final SimpleCommandExceptionType ERROR_SAME_SIZE = new SimpleCommandExceptionType(IChatBaseComponent.translatable("commands.worldborder.set.failed.nochange"));
    private static final SimpleCommandExceptionType ERROR_TOO_SMALL = new SimpleCommandExceptionType(IChatBaseComponent.translatable("commands.worldborder.set.failed.small"));
    private static final SimpleCommandExceptionType ERROR_TOO_BIG = new SimpleCommandExceptionType(IChatBaseComponent.translatable("commands.worldborder.set.failed.big", 5.9999968E7D));
    private static final SimpleCommandExceptionType ERROR_TOO_FAR_OUT = new SimpleCommandExceptionType(IChatBaseComponent.translatable("commands.worldborder.set.failed.far", 2.9999984E7D));
    private static final SimpleCommandExceptionType ERROR_SAME_WARNING_TIME = new SimpleCommandExceptionType(IChatBaseComponent.translatable("commands.worldborder.warning.time.failed"));
    private static final SimpleCommandExceptionType ERROR_SAME_WARNING_DISTANCE = new SimpleCommandExceptionType(IChatBaseComponent.translatable("commands.worldborder.warning.distance.failed"));
    private static final SimpleCommandExceptionType ERROR_SAME_DAMAGE_BUFFER = new SimpleCommandExceptionType(IChatBaseComponent.translatable("commands.worldborder.damage.buffer.failed"));
    private static final SimpleCommandExceptionType ERROR_SAME_DAMAGE_AMOUNT = new SimpleCommandExceptionType(IChatBaseComponent.translatable("commands.worldborder.damage.amount.failed"));
    ...
    private static int setDamageBuffer(CommandListenerWrapper commandlistenerwrapper, float f) throws CommandSyntaxException {
        ...
        if (worldborder.getDamageSafeZone() == (double) f) {
            ...
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.worldborder.damage.buffer.success", String.format(Locale.ROOT, "%.2f", f)), true);
            return (int) f;
        }
    }
    ...
    private static int setDamageAmount(CommandListenerWrapper commandlistenerwrapper, float f) throws CommandSyntaxException {
        ...
        if (worldborder.getDamagePerBlock() == (double) f) {
            ...
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.worldborder.damage.amount.success", String.format(Locale.ROOT, "%.2f", f)), true);
            return (int) f;
        }
    }
    ...
    private static int setWarningTime(CommandListenerWrapper commandlistenerwrapper, int i) throws CommandSyntaxException {
        ...
        if (worldborder.getWarningTime() == i) {
            ...
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.worldborder.warning.time.success", i), true);
            return i;
        }
    }
    ...
    private static int setWarningDistance(CommandListenerWrapper commandlistenerwrapper, int i) throws CommandSyntaxException {
        ...
        if (worldborder.getWarningBlocks() == i) {
            ...
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.worldborder.warning.distance.success", i), true);
            return i;
        }
    }
    ...
    private static int setCenter(CommandListenerWrapper commandlistenerwrapper, Vec2F vec2f) throws CommandSyntaxException {
        ...
        if (worldborder.getCenterX() == (double) vec2f.x && worldborder.getCenterZ() == (double) vec2f.y) {
            ...
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.worldborder.center.success", String.format(Locale.ROOT, "%.2f", vec2f.x), String.format("%.2f", vec2f.y)), true);
            return 0;
        } else {
    ...
    private static int setSize(CommandListenerWrapper commandlistenerwrapper, double d0, long i) throws CommandSyntaxException {
        ...
        if (d1 == d0) {
            ...
            if (i > 0L) {
                ...
                if (d0 > d1) {
                    commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.worldborder.set.grow", String.format(Locale.ROOT, "%.1f", d0), Long.toString(i / 1000L)), true);
                } else {
                    commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.worldborder.set.shrink", String.format(Locale.ROOT, "%.1f", d0), Long.toString(i / 1000L)), true);
                }
            } else {
